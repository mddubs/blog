...
On every machine I use, I begin by checking for an existing Tmux session.  If none are found, I create a new one and give it the same name as my user.  When I start a new task, no matter how small it may seem initially, I create a new Tmux window with a very descriptive name - say "check qa diskspace" or "calendar widget bug" for example.  Then within that window I customize an environment for the task at hand.  That could be Vim splits, Tmux splits, SSH connections, tailing files, or anything else.  The possibilites are endless, but generally it's just a single window with one split at the most.

Now that this workflow has become second nature, setting up a new environment takes only seconds and the benefits are immense.  The tools are so fast and powerful that the entire development environment becomes cheap.  Go ahead and run dozens of them without worrying at all about slowing anything down.  Keep one around that might pop up next month, it doesn't cost anthing.  Switching contexts becomes instantaneous, as long as your brain can keep up :)  
